			...
			if (receiveAID.size() == stationNumber) // 有效信号数量为设定值
			{
				...
				for (int i = 0; i < stationNumber; i++) {
					for (int j = 0; j < stationNumber; j++) {
						g_range_anchor(i, j) =
						    matrixBuff(receiveAID[i], receiveAID[j]);
						...
						if (isFirst) {
							...
							// 方案1：使用本次信号与上次信号的平均值
							//  g_range_anchor_avg[i][j] =
							//  (double)((g_range_anchor[i][j]+g_range_anchor_backup[i][j])/2.0f);
							// 方案2：使用本次信号与上次平均值的平均值
							g_range_anchor_avg(i, j) =
							    (double)((g_range_anchor(i, j) +
							              g_range_anchor_avg(i, j)) /
							             2.0f);
						}
				...
				// we have 4 ranges (A0 to A1, A0 to A2, A0 to A3, A1 to A2, A1
				// to A3, A2 to A3) calculate A0, A1, A2 and A3 coordinates (x,
				// y) based on A0 = 0,0 and assume A0-A1 makes the x-axis
				// 同时y轴垂直于x轴，A2, A3的y坐标为正
				int nNodes = stationNumber;
				int viewNode = 0;

				Eigen::MatrixXd twrdistance =
				    Eigen::MatrixXd::Zero(nNodes, nNodes);
				Eigen::MatrixXd transCoord =
				    Eigen::MatrixXd::Zero(nNodes, nDim);
				...
				mds(twrdistance, nNodes, viewNode,
				    transCoord); // Using multi-dimension scaling to estimation
				                 // the shape

				angleRotation(
				    transCoord, nNodes,
				    estCoord); // estCoord is the coordinates with a1 on the x
				               // axis and a2 in the first quadrant

				for (int i = 0; i < receiveAID.size();
				     i++) //  update positions of A0, A1, A2, A3
				{
					anchorArray(int(receiveAID[i]), 0) = estCoord(i, 0);
					anchorArray(int(receiveAID[i]), 1) = estCoord(i, 1);
					anchorArray(int(receiveAID[i]), 2) =
					    height; // 默认所有的基站都处于同一高度
...
void mds(Eigen::MatrixXd twrdistance, int nNodes, int viewNode,
         Eigen::MatrixXd& transCoord) {
	...
	Eigen::MatrixXd centeringOperator =
	    Eigen::MatrixXd::Identity(nNodes, nNodes) -
	    (Eigen::MatrixXd::Ones(nNodes, nNodes) / nNodes);

	Eigen::MatrixXd centeredDistSquared =
	    -0.5 * centeringOperator * distSquared * centeringOperator;

	Eigen::JacobiSVD<Eigen::MatrixXd> svd(
	    centeredDistSquared, Eigen::ComputeThinU | Eigen::ComputeThinV);
...
void angleRotation(Eigen::MatrixXd transCoord, int nNodes,
                   Eigen::MatrixXd& estCoord) {
	...
	for (int i = 0; i < nNodes; i++) {
		transdistance(i) = sqrt(transCoord(i, 0) * transCoord(i, 0) +
		                        transCoord(i, 1) * transCoord(i, 1));
	}

	Eigen::VectorXd currentAngle =
	    (transCoord.col(0).array() / transdistance.array()).acos();
	...
	rotationMatrix << cos(rotateAngle), -sin(rotateAngle), sin(rotateAngle),
	    cos(rotateAngle);
